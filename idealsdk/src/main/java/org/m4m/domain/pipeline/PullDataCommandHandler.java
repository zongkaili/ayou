...
class PullDataCommandHandler implements ICommandHandler {
	private static final String TAG = "PullDataCommandHandler";
    ...
    public void handle() {
        ...
        if (Frame.EOF().equals(frame)) {
            Logger.LOGD(TAG + " handle end of frame");
            input.drain(frame.getBufferIndex());
            Logger.LOGD(TAG + " handle end of frame end");
        }
        Logger.LOGD("8-");
    }
}
