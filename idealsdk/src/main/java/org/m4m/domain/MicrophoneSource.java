...
public class MicrophoneSource  implements  IMicrophoneSource{
    ...
    public void pull(Frame frame) {
        Logger.LOGD("MicrophoneSource domain pull "  + isStopped() + ",frame.length=" +  frame.getLength());
        if (!isStopped()) {
			...
			if(frame.getLength() < 0) {
                ...
                //meizu手机禁用录音权限后会出现错误，需要reset frame，然后继续走录制流程。
                frame.set(ByteBuffer.allocate(0), 0, 0, 0, 0, 0);
                //return;
            }
    ...
    public void stop() {
        ...
        Logger.LOGD("MicrophoneSource domain stop");
        isStopped = true;
        commandQueue.queue(Command.HasData, 0);
    }
